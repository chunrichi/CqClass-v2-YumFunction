...
exports.main = async(event, context) => {
  ...
  if (!ifHaveThisTerm) {
    ...
    await rp({
      ...
      // console.log('\n\n\nbody', body)
      classMessage = JSON.parse(body)
    }).catch(error => {
      classMessage.messageStatus = false
      classMessage.errorMessage = 'request api error'
    })

    // 如果数据请求成功 则写入数据库
    if (classMessage.messageStatus) {
      // 将课表信息添加进 kb_classtables 中
      await db.collection('kb_classtables').add({
        data: {
          term: term,
          uid: username,
          termClassTables: classMessage.classMessage
        }
      })
      // 返回
      ...
      for (let course of classMessage.classMessage) {
        let courseObject = {}
      ...
      returnMessage.errorMessage = classMessage.errorMessage
      return returnMessage
    }
  }
}